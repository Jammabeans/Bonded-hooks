...
const master = new ethers_1.ethers.Contract(masterAddr, MasterControlABI, wallet);
const settingsAddr = process.env.SETTINGS_ADDRESS;
const settings = settingsAddr ? new ethers_1.ethers.Contract(settingsAddr, SettingsABI, wallet) : null;
const gasRebateAddr = process.env.GAS_REBATE_ADDRESS;
const gasRebate = gasRebateAddr ? new ethers_1.ethers.Contract(gasRebateAddr, GasRebateABI, wallet) : null;
const bidManagerAddr = process.env.BID_MANAGER_ADDRESS;
const bidManager = bidManagerAddr ? new ethers_1.ethers.Contract(bidManagerAddr, BidManagerABI, wallet) : null;
const degenPoolAddr = process.env.DEGEN_POOL_ADDRESS;
const degenPool = degenPoolAddr ? new ethers_1.ethers.Contract(degenPoolAddr, DegenPoolABI, wallet) : null;
...
// Settings keys
const KEY_DEFAULT_REFUND = ethers_1.ethers.keccak256(ethers_1.ethers.toUtf8Bytes("defaultRefundWei"));
const KEY_EPOCH_SECONDS = ethers_1.ethers.keccak256(ethers_1.ethers.toUtf8Bytes("epochSeconds"));
const KEY_POINTS_PER_WEI = ethers_1.ethers.keccak256(ethers_1.ethers.toUtf8Bytes("pointsPerWei"));
// Helpers
async function resolveRefundForPool(poolId, hookAddress) {
...
function computeEpoch(epochSeconds) {
    const now = Math.floor(Date.now() / 1000);
    return Math.floor(now / epochSeconds);
}
// Read bidder info from BidManager
async function readBidInfo(bidder) {
    if (!bidManager)
        return null;
    try {
        const bid = await bidManager.getBid(bidder);
...
async function matchBiddersForAmount(amountWei) {
    if (!bidManager || candidateBidders.length === 0)
        return [];
    const infos = [];
    ...
    infos.sort((a, b) => {
        ...
        if (b.totalBidAmount !== a.totalBidAmount)
            return (b.totalBidAmount > a.totalBidAmount) ? 1 : -1;
        return 0;
    });
...
async function readPointsPerWei() {
    if (settings) {
        try {
    ...
    return BigInt(1000000000000n);
}
master.on("PoolRebateReady", async (txOrigin, trader, poolId, poolTotalFeeBips, baseGasPrice, event) => {
    try {
