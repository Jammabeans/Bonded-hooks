...
exports.handlePoolRebateReady = handlePoolRebateReady;
...
function getDegenPool() {
    if (degenPool)
        return degenPool;
    if (!degenPoolAddr || !ethers || !ethers.Contract)
        return null;
    try {
        const providerOrSigner = wallet || provider || undefined;
...
function getGasRebate() {
    if (gasRebate)
        return gasRebate;
    if (!gasRebateAddr || !ethers || !ethers.Contract)
        return null;
    try {
        const providerOrSigner = wallet || provider || undefined;
...
const DEFAULT_POINTS_PER_WEI = BigInt(1000000000000n);
...
async function readEncryptedBidPlain(bidder) {
    const bm = getBidManager();
    if (!bm)
        return null;
    try {
        const res = await bm.getBidEncrypted(bidder);
        if (!res)
            return null;
...
function matchBiddersFromInfos(infosInput, amountWei) {
...
async function handlePoolRebateReady(payload) {
    try {
        ...
        if (LIVE) {
            ...
            if (!bm) {
                ...
                try {
                    ...
                    const tx = await bm.finalizeEpochConsumeBids(epoch, biddersPlain, consumedAmounts.map((s) => BigInt(s).toString()));
                    await tx.wait();
