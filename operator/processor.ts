import { matchBiddersFromInfos, BidInfo } from "./matcher";

const KEY_DEFAULT_REFUND = (s: string) => s; // placeholder, settings keys should be keccak256 in real usage
...
export type Deps = {
  ...
  bidManager?: any;
  degenPool?: any;
  candidateBidders?: string[];
  nowSeconds?: number;
};

export async function processRebateEvent(
  txOrigin: string,
  trader: string,
  poolId: bigint,
  poolTotalFeeBips: bigint,
  baseGasPrice: bigint,
  deps: Deps
) {
  ...
  if (settings && typeof settings.getHookRefundFor === "function") {
    try {
      ...
      if (v && v.toString() !== "0") refundWei = BigInt(v.toString());
    } catch {
      // ignore
    }
  }
  ...
  for (const b of candidateBidders) {
    if (!bidManager || typeof bidManager.getBid !== "function") break;
    try {
      const bid: any = await bidManager.getBid(b);
      if (!bid) continue;
      infos.push({
        bidder: bid.bidder,
  ...
  let pointsPerWei = BigInt(1_000_000_000_000n);
  let cooldownSeconds = 0;
  if (settings && typeof settings.getUint === "function") {
    try {
      const p = await settings.getUint(KEY_DEFAULT_REFUND("pointsPerWei"));
      if (p && p.toString() !== "0") pointsPerWei = BigInt(p.toString());
    } catch {}
    try {
      const c = await settings.getUint(KEY_DEFAULT_REFUND("bidderCooldownSeconds"));
      if (c && c.toString() !== "0") cooldownSeconds = Number(c.toString());
    } catch {}
  }

  // Match bidders
  const matches = matchBiddersFromInfos(infos, refundWei, { pointsPerWei, cooldownSeconds, nowSeconds: now });
