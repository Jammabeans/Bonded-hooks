import { matchBiddersFromInfos } from "../matcher";

function make(addr: string, total: bigint, maxEpoch = BigInt(0), rush = 0, minMint = BigInt(0), lastUpdated = 0) {
  return {
    bidder: addr,
...
describe("matchBiddersFromInfos", () => {
  test("single bidder covers full amount", () => {
    ...
    const res = matchBiddersFromInfos(infos as any, BigInt(500));
  ...
  test("filters by minMintingRate (pointsPerWei)", () => {
    ...
    const res = matchBiddersFromInfos(infos as any, BigInt(100), { pointsPerWei: BigInt(1) });
  ...
  test("filters by cooldownSeconds", () => {
    ...
    const res = matchBiddersFromInfos(infos as any, BigInt(100), { cooldownSeconds: 60, nowSeconds: now });
    expect(res.some(r => r.bidder === "A")).toBe(false);
