...
describe("DegenAVS_COFHE integration (mock cofhejs + ethers)", () => {
  ...
  test("dry-run: handlePoolRebateReady logs planned minting and does not call finalize", async () => {
    ...
    jest.doMock("cofhejs", () => ({
      init: async () => {},
      unseal: async (val: any, _type?: string) => {
    ...
    const mockGetBidEncrypted = jest.fn(async (addr: string) => {
    ...
    const entry = logs.find((l: any[]) => l[0] && String(l[0]).includes("Planned bidder minting"));
  ...
  test("LIVE mode: handlePoolRebateReady calls finalizeEpochConsumeBids and waits for tx", async () => {
    ...
    jest.doMock("cofhejs", () => ({
      init: async () => {},
      unseal: async (val: any, _type?: string) => (val && val._plain ? val._plain : val)
    }));
    ...
    const mockFinalize = jest.fn().mockImplementation(async (epoch: number, bidders: string[], consumed: string[]) => {
    ...
    expect(call.consumed.every((c: string) => /^\d+$/.test(String(c)))).toBe(true);
  });
});