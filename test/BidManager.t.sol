...
contract BidManagerTest is Test {
    ...
    function testCreateAndTopUpAndConsume() public {
        ...
        uint256 amt1_after = bid1_after.amountWei;
        ...
        assertEq(bm.getBid(bidder1).amountWei, 1.5 ether - 0.8 ether);
        assertEq(bm.getBid(bidder2).amountWei, 2 ether - 1.2 ether);
    ...
    function testMinBidEnforced() public {
        ...
        bm.createBid{value: 0.005 ether}(10);
    }

    function testCannotCreateSecondBidForSameAddress() public {
        vm.deal(bidder1, 3 ether);
        vm.prank(bidder1);
        bm.createBid{value: 1 ether}(10);

        vm.prank(bidder1);
        vm.expectRevert(bytes("Bid exists"));
        bm.createBid{value: 1 ether}(20);
    }

    function testPendingRushAppliedTiming() public {
        vm.deal(bidder1, 3 ether);
        vm.prank(bidder1);
        bm.createBid{value: 1 ether}(50); // initial rush 50

        // bidder sets pending rush with effectiveEpoch = 5
        vm.prank(bidder1);
        bm.updateRushFactor(100, 5); // pendingRushFactor = 100, lastUpdatedEpoch = 5

        // Owner enables settlement role
        bm.setSettlementRole(address(this), true);

        // finalize epoch 5 => should NOT apply because lastUpdatedEpoch == epoch (apply only if lastUpdatedEpoch < epoch)
        address[] memory biddersArr = new address[](1);
        biddersArr[0] = bidder1;
        uint256[] memory consumed = new uint256[](1);
        consumed[0] = 0;
        bm.finalizeEpochConsumeBids(5, biddersArr, consumed);
        assertEq(bm.getBid(bidder1).rushFactor, 50);

        // finalize epoch 6 => should apply
        bm.finalizeEpochConsumeBids(6, biddersArr, consumed);
    ...
    function testOwnerRecoverBidTransfersAndFailsOnInsufficient() public {
        ...
        assertEq(bm.getBid(bidder1).amountWei, 1 ether);
    ...
    function testEventsOnCreateTopUpAndConsume() public {
        ...
        emit BidCreated(bidder1, 1 ether, 42);
        vm.prank(bidder1);
        bm.createBid{value: 1 ether}(42);
